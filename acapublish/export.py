...
def adjust_mmd_for_word(source_file, export_file, settings):
    ...
    if settings.SEPERATE_TABLES:
        for a in re.finditer(r"<STARTTABLE>(.*?)<ENDTABLE>", text, re.DOTALL):
            ...
            count += 1
        table_text = '\n\r<div style="page-break-after: always;"></div>'.join(
            tables)
...
def create_latex_variables(settings, destination):
    ...
    if hasattr(settings, "DATE"):
        text = text.replace("DATE", settings.DATE)
    else:
        text = text.replace("\def\mydate{DATE}", "")
...
def process_md(folder, root, export_file, settings):
    ...
    cite = CitationProcessor(settings.BIB_FILE,
                             ...
                             gender_override=settings.GENDER_OVERRIDE,
                             gender_locale=settings.GENDER_LOCALE)
    ...
    for x, a in enumerate(re.finditer(r"\[\#(.*?)\]", text)):
        ...
        if settings.SHORT_CITE:
            ...
            ref_as_year_format = "{0} {1}"

            if year_only:
                if use_ref_as_year:
                    new_ref = ref_as_year_format.format(name, new_ref)
                else:
    ...
    if settings.DOUBLE_LINK_BREAKS:
        text = text.replace("|\n\r", "--TABLE--BREAK--")
        text = text.replace("|\r\n", "--TABLE--BREAK--")
        text = text.replace("\n\r", "\n\r\n\r")
        text = text.replace("\r\n", "\r\n\r\n")
        text = text.replace("--TABLE--BREAK--", "|\r\n")

    QuickText(text=text).save(export_file)
    cite.export_gender(settings.SLUG)


def convert_mmd_to_word(mmd_file, word_file, settings):
    extra_args = ()
    if hasattr(settings, "DOCX_TEMPLATE") and settings.DOCX_TEMPLATE:
        extra_args = ("--reference-doc={0}".format(settings.DOCX_TEMPLATE),)
