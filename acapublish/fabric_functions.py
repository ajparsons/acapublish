...
def googlesync():
    ...
    d.sync_google_doc_folder(settings.MARKDOWN_FOLDER,
                             settings.GOOGLE_DRIVE_PATH)


def compile(create_pdf=True, create_word=True, only=None):
    """
    ...
    if hasattr(settings, "SCRIVENER_FOLDER") and settings.SCRIVENER_FOLDER:
        scriv_to_md(settings.SCRIVENER_FOLDER,
                    settings.SCRIVENER_ROOT, settings.MARKDOWN_FOLDER)
    ...
    if create_word:
        ...
        convert_mmd_to_word(r"_compile\content_word.mmd",
                            r"_outputs\{0}.docx".format(slug),
                            settings)
    if create_pdf:
        adjust_md_for_latex(r"_compile\content.mmd",
                            r"_compile\content_latex.mmd")
        os.system(r'multimarkdown -b -t latex _compile\content_latex.mmd')
        ...
        os.system(
            r'cd _compile && pdflatex master.tex && bibtex master.aux & pdflatex master.tex')
        shutil.copyfile(r"_compile\master.pdf",
                        r"_outputs\{0}.pdf".format(slug))
